---
name: Alex Cerqueira de Araujo
full_name: Alexandra Cerqueira de Araujo
role: Senior Bioinformatician
status: current
joined: "September 2024"
scopus_author_id: "57463627000"
photo: /wp-content/uploads/2026/08/alexcda.jpg
title: Alex Cerqueira de Araujo
redirect_from:
  - /our-group/alex-cerqueira-de-araujo/
links:
  - label: Google Scholar
    url: https://scholar.google.com/citations?user=8h6Wd2oAAAAJ&hl=en
  - label: ORCID
    url: https://orcid.org/0000-0002-9273-9886
  - label: ResearchGate
    url: https://www.researchgate.net/profile/Alex-Cerqueira-De-Araujo?ev=hdr_xprf
  - label: GitHub
    url: https://github.com/Alex-cdea
  - label: LinkedIn
    url: https://www.linkedin.com/in/alex-cerqueira-de-araujo-785490248
position: Senior bioinformatician
projects:
  - title: Comparative genomics and evolution of hymenopterans
    text: |-
      One of my main goals is to build a genome-scale evolutionary genomics pipeline (300+ assemblies) covering assembly QC, repeat annotation, structural/functional gene annotation, orthology, phylogenetics, synteny, and selection analysis.
#  - title: Another one
#    text: |-
#      testing
past_projects:
  - title: Comparative and evolutionary genomics of ticks
    text: |-
      I was a key contributor to a comparative genomics project generating chromosome-level assemblies for four *Ixodes* tick species, including *I. ricinus*, the main vector of Lyme disease, and using them to study tick genome architecture and long-term evolutionary history — a foundational piece of work that gave the field genomic resources for this genus. Building on that work, I turned to horizontal gene transfer (HGT) from bacteria into tick genomes. I identified several genes originating from HGTs. Among them, a bacterial gene (AnmK), normally involved in recycling bacterial cell-wall material, was acquired by the common ancestor of all ticks and has since become functionally integrated into tick biology.
        ![]({{ site.baseurl }}/wp-content/uploads/2025/01/images_for_labwebsite_ticks.png)
      Figure legend: Evolution of tick genomes. Top: Chromosome syntenies between *Ixodes ricinus* (orange) and *Ixodes scapularis* (purple-blue). Centre left: phylogeny of *Ixodidae*. Node circles show the gene family expansions (blue: expanding, red: contracting). The numbers represent the number of expanded (+) / contracted (-) gene families and the number of gained (green) gene families. Symbols represent □ *Ixodidae*, Δ Metastriata, ○ Prostriata, ∇ *Ixodes hexagonus*, ⋄ Other *Ixodes*. Centre right: GO terms associated to expanded families in the *Ixodidae* tree (centre left). Symbols correspond to symbols on the tree. Bottom: Gene tree of cytosolic sulfotransferases. Label prefixes correspond to each species: the tick *I. ricinus* (SFT, labels in blue), the spider *Parasteatoda tepidariorum* (ptepid), the horse-shoe crab *Limulus polyphemus* (limulus), human and *Drosophila melanogaster* (dmel). The chromosomal localization (scaffold number) is indicated in the first outer circle. The next outer circle are bar-charts of the number of coding exons (dark-gray filling) and 5′ UTR-only exons (orange filling).
  - title: Evolution of viral domestication in Campopleginae wasps
    text: |-
      During my PhD, I studied how parasitoid wasps domesticate viruses, turning them into weapons against the insects they parasitize. Working mainly on Campopleginae wasps like *Venturia canescens*, I showed that conserved, domesticated viral transcriptional machinery drives the production of virus-like particles (VLPs), which help wasp larvae suppress their host's immune defenses. I described two other endogenous nudiviruses, one endogenised in *Campoplex capitator*, the other one endogenised in *Campoplex nolae*, and traced how this nudivirus-derived VLP system evolved in this group of wasps. In a comparative follow-up, I found that unrelated domesticated viral lineages — nudivirus-derived VLPs in *Campoplex* wasps and ichnoviruses — had independently and convergently recruited the same proteins (transcribed from *repeat element* genes), suggesting there's a shared evolutionary "toolkit" that different wasp lineages draw on when domesticating viruses.
        ![]({{ site.baseurl }}/wp-content/uploads/2025/01/images_for_labwebsite_wasp.png)
      Figure legend: Evolution of Nudiviruses endogenised in *Campopleginae* wasps. Top left: dNdS ratios of viral (red, green, pink and blue) and wasp genes (grey). Top right: Syntenies of viral genes endogenised in two different species of wasps (*Venturia canescens* and *Campoplex capitator*). Viral genes are colored (yellow for lost genes by pseudogenisation) while wasp genes are white/grey. Bottom left: phylogeny of *Naldaviricetes*. Nudiviruses endogenised in *Campopleginae* are written in red (CnolaENV: Campoplex nolae Endogenous Nudivirus, CcapiENV: Campoplex capitator Endogenous Nudivirus, VcENV: Venturia canescens Endogenous Nudivirus). Bottom right: expression kinetic of viral genes and virulence factors (wasp genes envelopped in Virus-Like Particles - VLPs) in *Venturia canescens*. P1 to P5 refer to nymphal instars, where the VLP production is low compared to the adults.
background:
- 'Sept 2024–present: Postdoctoral researcher, UCL, UK'
- 'Apr 2024-May 2024: Molecular phylogeny course - advanced, Ecole SupAgro Montpellier, France'
- 'Jan 2024-Feb 2024: Visiting researcher, "Investigation of new cases of horizontal gene transfer in ticks", Institute of Parasitology, Biology Centre, Czech Academy of Sciences, Czech Republic'
- 'Jan 2023–Jun 2024: Postdoctoral researcher, "Comparative and evolutionary genomics of ticks", INRAE-Oniris, France'
- 'Feb 2019-Mar 2019: Bioinformatic courses, "UNIX initiation: big data manipulation using UNIX", Agrocampus, Rennes, France'
- 'Sept 2018–Jul 2022: PhD in Science (Biology), "Evolution of recently domesticated viruses in Campopleginae parasitoid wasps", Institut de recherche sur la biologie de l’insecte, CNRS UMR 7261, Université de Tours, France'
- 'Feb 2018-Jul 2018: Master 2 internship, "Immune plasticity of Armadillidium vulgare (pillbug)", Ecologie et Biologie des interactions laboratory - UMR CNRS 7267, Université de Poitiers, France'
- 'Feb 2017-Jul 2017: Master 2 internship, "Immune plasticity of Armadillidium vulgare (pillbug)", Ecologie et Biologie des interactions laboratory - UMR CNRS 7267, Université de Poitiers, France'
- 'May 2016-Jul 2016: Master 1 internship, "Effects of cyanotoxins on zooplankton communitie", Ecologie, Systematique et Evolution laboratory - UMR 8079, Université d’Orsay, France'
- 'Sept 2015–Jun 2018: MSc in Evolutionary Ecology, "The chemical crown of social insect wasps", Université Paris-Saclay, France'
- 'May 2015-Jul 2015: Internship, "Evolution of plant-pollinator relationship throughout the urbanization gradient", Ecologie, Systematique et Evolution laboratory - UMR 8079, Université d’Orsay, France'
- 'Sept 2012–Jun 2015: Bachelor of science degree in biology, Université de Poitiers, France'

interests: |-
  My work focuses on the evolution of organisms, especially the evolution of arthropod genomes.

  I use genomic, transcriptomic and proteomic data to find evolutionary patterns among species. These patterns can take several forms, from single nucleotide polymorphisms to chromosome structural changes — they help me better understand the link between genome and phenotype, and to draw evolutionary relationships between organisms. In combination with phenotypic features, genomic features can also be used to predict species life-history traits and evaluate species susceptibility to, for example, environmental change.

background_text: |-
  I am a French researcher specialising in evolutionary biology and bioinformatics. I earned my PhD in molecular biology at the University of Tours (France), supervised by Prof Huguet and Dr Josse.

  My research background focuses on evolutionary biology, leveraging genomic data to uncover insights into the evolutionary processes shaping organisms. During my PhD I explored the evolutionary dynamics of viral symbiosis in parasitoid wasps, providing insights into the interactions between wasp and viral genomes and their impact on wasp biology. Afterwards, through comparative genomics, I studied tick genome evolution, which highlighted features such as gene expansions that support their parasitic lifestyle. Beyond my primary research projects, I've shared my bioinformatics expertise with students and researchers, contributing to multiple publications, and taught genetics and basic evolution concepts to undergraduates.

  My study approaches integrate molecular and evolutionary biology, bioinformatics, and functional and comparative genomics.
other_interests: I like arthropods! so I draw them and make 3D models out of them. 
  ![]({{ site.baseurl }}/wp-content/uploads/2026/10/board_wasp.png)
publications_text: |-
  *Book chapter*

  1. Hulot F and Cerqueira de Araujo A. "Effets des cyanotoxines sur les communautés zooplanctoniques en milieu lentique." In: Bernard, C., Mougin, C. and Pery, A. (eds) *Écotoxicologie, des communautés au fonctionnement des écosystèmes*. ISTE, 2017, pp. 137–152.
contact: '[alexandra.araujo@ucl.ac.uk](mailto:alexandra.araujo@ucl.ac.uk)'
---
